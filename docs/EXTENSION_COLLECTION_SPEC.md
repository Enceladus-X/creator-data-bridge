...
- 상태: `0.2.0` 구현 완료, 실계정 설치 smoke test 대기
- 기준일: 2026-07-12
...
- YouTube Studio, TikTok, Instagram, X는 사용자가 각 서비스에 로그인해 있으면 확장프로그램이 필요한 탭을 열고 화면에 표시된 데이터를 읽는다.
- 사용자가 플랫폼 탭을 미리 열어둘 필요는 없다.
- 열린 플랫폼 탭은 계정 핸들 탐색에만 참고하고, 실제 수집에는 별도의 임시 탭을 만든다.
- 임시 탭은 탭 표시줄에 나타날 수 있지만 기본적으로 비활성 상태로 열고, 수집이 끝나면 확장프로그램이 만든 탭만 닫는다.
- YouTube는 Studio 화면을 기본 로컬 수집 경로로 사용하고, 기존 공식 API는 선택적 고급 분석 경로로 유지한다.
...
- 사이드 패널에서 `수집하고 CSV 다운로드` 한 번으로 활성화한 플랫폼을 순차 수집한다.
...
| YouTube | Studio 대시보드와 콘텐츠/Shorts 표 | 채널명, 구독자, 최근 28일 조회·시청 시간, 영상 ID/URL, 제목, 날짜, 공개 상태, 조회, 댓글 | Studio 표에서 좋아요 미제공, selector 변경 가능성 |
...
1. 사용자가 확장프로그램을 설치한다.
2. 사이드 패널의 토글로 YouTube, TikTok, Instagram, X 중 수집할 플랫폼을 활성화한다.
3. 활성화 설정과 플랫폼별 최대 콘텐츠 수는 `chrome.storage.local`에 저장한다.
4. 사용자가 `수집하고 CSV 다운로드`를 처음 누를 때 선택한 도메인의 권한을 한 번에 요청한다.
5. 열린 프로필 탭, 저장된 핸들, 로그인 홈 화면 순으로 계정을 감지한다.
6. 확장프로그램은 비밀번호를 받거나 쿠키 값을 직접 읽지 않는다.

### 4.2 원클릭 수집

1. 사용자가 `수집하고 CSV 다운로드`를 누른다.
...
7. 성공한 행이 하나라도 있으면 최신 실행 CSV를 자동으로 내려받는다.
8. 사용자는 `CSV 저장`으로 같은 결과를 다시 내려받을 수 있다.
...
flowchart LR
    ...
    N --> IDB["IndexedDB 스냅샷"]
    IDB --> EX["CSV 내보내기"]
    SW -. 선택적 고급 분석 .-> API["YouTube API 백엔드"]
```
...
| 탭 관리자 | 열린 탭에서 계정 힌트 탐색, 임시 탭 생성, 준비 상태 확인과 정리 |
...
- 열린 플랫폼 탭의 URL에서 계정 핸들을 감지할 수 있다.
- 사용자 탭은 이동하거나 닫지 않고 실제 수집은 별도 임시 탭에서 수행한다.
...
원클릭 수집을 시작할 때 활성화한 플랫폼의 도메인 권한만 한 번에 요청한다. `<all_urls>`, `cookies`, `webRequest` 권한은 사용하지 않는다. 로그인 세션은 Chrome이 페이지를 불러올 때 정상적으로 적용하며 확장프로그램은 쿠키 값을 직접 읽지 않는다.
...
- 완료 영역: 수집 행 수, 누락 지표 수, CSV 재저장 버튼
...
- YouTube, TikTok, Instagram, X에서 각각 현재 콘텐츠 수집
...
- 사용자가 플랫폼 탭을 미리 열지 않아도 네 플랫폼 수집을 시작할 수 있다.
- 로그인된 loorbit 계정의 YouTube, TikTok, Instagram, X 콘텐츠가 수동 검증 결과와 일치한다.
