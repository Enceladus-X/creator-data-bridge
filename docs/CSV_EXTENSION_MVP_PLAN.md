...
> 로그인된 Chrome에서 `수집하고 CSV 다운로드`를 누르면 활성화한 플랫폼의 프로필과 콘텐츠 지표를 수집하고, 누락 상태가 명확한 CSV를 자동 저장한다.
...
7. 수집 완료 후 Excel 호환 CSV가 자동으로 내려받아진다.
