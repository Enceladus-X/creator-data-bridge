...
3. `수집`을 눌러 데이터를 가져옵니다.
4. 최초 실행이면 선택한 사이트의 접근 권한을 한 번에 허용합니다.
5. 전체 실행 로그에서 플랫폼별 진행 상황과 오류를 확인합니다.
6. 수집이 끝나면 `CSV 다운로드`를 눌러 UTF-8 BOM CSV를 저장합니다.
...
확장프로그램은 `127.0.0.1:48765`의 로컬 명령 서버를 30초 간격으로 확인합니다. 사용자가 신뢰하는 데스크톱 제어판이 서버를 잠시 열면, 설정에서 활성화한 플랫폼을 수집하고 CSV를 서버에 반환할 수 있습니다. 서버가 꺼져 있을 때는 아무 데이터도 전송하지 않으며 기존 사이드 패널 동작에는 영향이 없습니다. 이 기능을 적용한 뒤에는 `chrome://extensions`에서 개발 빌드를 한 번 다시 로드해야 합니다.

사이드 패널 상단의 대시보드 아이콘을 누르면 최신 로컬 수집 데이터를 시각화한 전체 대시보드가 열립니다. 대시보드는 플랫폼별 조회·좋아요·댓글 합계, 조회 상위 콘텐츠, 검색·필터·정렬 가능한 콘텐츠 표, 플랫폼 비교 막대그래프와 CSV 재다운로드를 제공합니다. 개요의 플랫폼 아이콘은 각 업로드 또는 작성 화면을 새 탭으로 엽니다.

설정에서 한국어와 English를 전환할 수 있습니다. 언어와 플랫폼 선택은 사이드 패널과 대시보드가 공유하며 로컬에 저장됩니다. 실행 로그도 선택한 언어로 표시하고 복사할 수 있습니다.
...
Chrome MV3 사이드 패널, 선택적 사이트 권한, 플랫폼 토글 설정, 한국어/영어 UI, 임시 탭 오케스트레이터, YouTube Studio·TikTok Studio·Instagram Reel·X 프로필 수집기, IndexedDB 저장, 상세 실행 로그, 부분 성공과 재시도가 구현되어 있습니다. 전체 대시보드는 같은 로컬 레코드를 플랫폼·콘텐츠 단위로 시각화하고 CSV 다운로드와 업로드 화면 바로가기를 지원합니다. `0`과 미제공 지표를 구분하며 개인정보를 제거한 DOM fixture와 CSV 계약 테스트를 포함합니다. 댓글 작성, DM 관리와 자동 업로드는 현재 CSV MVP 범위에 포함하지 않습니다.
